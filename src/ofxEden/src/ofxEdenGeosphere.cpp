...
ofxEdenGeosphere& ofxEdenGeosphere::allocate(int _width, int _height)
{
	...
	averageTexture.allocate(_width,_height,GL_RGB32F);
    
    for (int i = 0; i < 2; i++){
        fbo[i].allocate(width,height,GL_RGB32F);
        fbo[i].begin();
        ofClear(255,255);
        fbo[i].end();
    }
	...
	normalFbo.allocate(width, height,GL_RGB32F);
	...
	normalTexture.allocate(_width,_height,GL_RGB32F);
	
    cout << "- noise shader" << endl;
	noiseFbo.allocate(width, height,GL_RGB32F );
	...
	noiseTexture.allocate(_width,_height,GL_RGB32F );
    ...
    conditionsFbo.allocate(width, height,GL_RGB32F);
...
ofxEdenGeosphere& ofxEdenGeosphere::setTotalFrames(int n, ofTexture& cleanTexture)
{
	...
	for (int i = 0; i < totalFrames; i++){
		frames[i] = ofTexture();
		frames[i].allocate(width,height,GL_RGB32F);
